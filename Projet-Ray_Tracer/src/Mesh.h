...
public:
    ...
    RayTriangleIntersection intersect( Ray const & ray ) const {
        ...
        for( unsigned int t = 0 ; t < triangles.size() ; ++t ) {
            Triangle triangle( vertices[triangles[t][0]].position * triangleScaling, vertices[triangles[t][1]].position * triangleScaling, vertices[triangles[t][2]].position * triangleScaling);
            RayTriangleIntersection intersection = triangle.getIntersection(ray);
            if( intersection.intersectionExists && intersection.t < closestIntersection.t && intersection.t > 0.001 ) {
