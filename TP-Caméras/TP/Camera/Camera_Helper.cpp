...
void Camera_Helper::clipAngle180(float& _angle)
{
	if (_angle > 180.f)
	{
		_angle -= 360.f;
	}
	else if (_angle < -180.f)
	{
		_angle += 360.f;
	}
}

glm::vec3 Camera_Helper::ProjectVectorOnPlane(const glm::vec3& _vector, const glm::vec3& _planeNormal)
{
    return _vector - glm::dot(_vector, _planeNormal) * _planeNormal;
}


float Camera_Helper::interpolate(float _ratio, InterpolationType _type) {
    switch (_type) {
        case InterpolationType::LINEAR:
            return _ratio;
        
        case InterpolationType::COS:
            return (1 - std::cos(_ratio * M_PI)) / 2;
        
        case InterpolationType::EXP:
            return std::pow(2, 10 * (_ratio - 1));
