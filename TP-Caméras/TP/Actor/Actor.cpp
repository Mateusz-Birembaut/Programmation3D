...
void Actor::updateViewAndDraw(Camera& _camera, GLuint _matrixID, GLuint _modelMatrixID)
{
    if (m_showTarget)
    {
        m_model.updateViewAndDraw(_camera, _matrixID, _modelMatrixID);
        if (_camera.getIsTrackingTarget()){
			m_moveTarget = true;
			_camera.trackActor(m_position, m_rotation);
        }
    }
}
