...
static const std::string DEFAULT_MESH_FILENAME ("Resources/Models/rhino2.off");
...
struct Light {
    ...
    void setupCameraForShadowMapping(glm::vec3 scene_center , float scene_radius) {
        ...
        glm::mat4 lightViewMatrix = glm::lookAt(lightPos, scene_center, upVector);

        float nearPlane = 1.0f;
        float farPlane = scene_radius * 2.0f;
        ...
        depthMVP = lightProjectionMatrix * lightViewMatrix;
    }
...
struct Scene{
    ...
    void render() {
        ...
        for (int i = 0; i < scene_lights.size(); ++i) {
            Light & light = scene_lights[i];
            std::string lightUniform = "lightSources[" + std::to_string(i) + "]";
            shaderProgramPtr->set(lightUniform + ".position", light.m_position);
            shaderProgramPtr->set(lightUniform + ".color", light.m_color);
            shaderProgramPtr->set(lightUniform + ".intensity", light.m_intensity);
            shaderProgramPtr->set(lightUniform + ".isActive", 1);
            shaderProgramPtr->set(lightUniform + ".depthMVP", light.depthMVP);
            glActiveTexture(GL_TEXTURE0 + light.shadowMapTexOnGPU);
            glBindTexture(GL_TEXTURE_2D, light.shadowMap->depthMapTexture);
            std::string shadowMapUniform = "shadowMap[" + std::to_string(i) + "]";
            shaderProgramPtr->set(shadowMapUniform, static_cast<int>(light.shadowMapTexOnGPU));
        }

        // Set the number of lights
        shaderProgramPtr->set("number_of_lights", static_cast<int>(scene_lights.size()));
        ...
        shaderProgramPtr->set ("material.albedo", glm::vec3( 0.4 , 0.4 , 0.8 ) );
        shaderProgramPtr->set ("material.shininess" , (float) 50.f );
...
void initScene (const std::string & meshFilename) {
    ...
    // Setup lights :
    {
        ...
        for (int j = 0; j < 3; ++j) { // 3 lights
            ...
            std::cout << newLight.m_position.x << " " << newLight.m_position.y << " " << newLight.m_position.z << std::endl;
        }
        ...
        std::cout << " > " << scene.scene_lights.size() << " lights created" << std::endl;
